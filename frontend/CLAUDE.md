...
- No inline `<svg>` markup in components. Each icon/graphic is its own standalone SVG file under `public/icons/` (own `viewBox`; fill/stroke color is irrelevant since only the shape's alpha is used — keep it a neutral `#000`). Reference it with the shared `.icon-mask` utility (`src/index.css`) on a `<span>`, driving color via the normal Tailwind `text-*` class and pointing the mask at the file via the `--icon-url` custom property: `<span class="icon-mask h-6 w-6 text-indigo-600 [--icon-url:url(/icons/user-circle.svg)]" aria-hidden="true"></span>`. This keeps one file per shape — never duplicate a file just to change color.
